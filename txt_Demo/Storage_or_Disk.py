...
# Fetch disk data for Windows (only C:)
# def fetch_windows_disk_data():
#     disk_usage = psutil.disk_usage('C:/')
#     total = disk_usage.total / (1024 ** 3)  # Convert bytes to GB
#     used = disk_usage.used / (1024 ** 3)  # Convert bytes to GB
...
# Fetch disk data for Windows (using psutil)


# Fetch disk data for Windows (only C:/ drive)
def fetch_windows_disk_data():
    try:
        # Fetching the disk usage for the 'C:/' drive
        disk_usage = psutil.disk_usage('C:/')
        total = disk_usage.total / (1024 ** 3)  # Convert from bytes to GB
        used = disk_usage.used / (1024 ** 3)    # Convert from bytes to GB
        free = disk_usage.free / (1024 ** 3)    # Convert from bytes to GB

        return {
            "disk_total": round(total, 2),  # Rounded to 2 decimal places for better readability
            "disk_used": round(used, 2),    # Rounded to 2 decimal places for better readability
            "disk_free": round(free, 2)     # Rounded to 2 decimal places for better readability
        }
    except Exception as e:
        print(f"Error fetching disk usage from Windows: {e}")
        return None
...
def fetch_api_data(host_name):
    ...
    if response.status_code == 200:
        ...
        if 'message' in api_data:
            ...
            print("Key 'message' not found in API response.")
            return None
